defmodule TalarWeb.ChatLive.Index do
  ...
  def handle_event(
    ...
    with {:ok, _chat_user} <-
      ...
      Accounts.broadcast(
        %Events.ReceivedMessage{chat_id: current_chat_id, timestamp: DateTime.utc_now()},
        receiver_user_id
      )
  ...
  def handle_event("open_chat", %{"username" => username}, socket) do
    ...
    {:ok, chat} =
      case Chats.get_chat_by_user_preloaded(current_user.id, user.id) do
        nil -> Chats.create_chat(%{user_id_1: current_user.id, user_id_2: user.id})
        chat -> {:ok, chat}
      end

    chat =
      if Ecto.assoc_loaded?(chat.chat_users) do
  ...
  def handle_info(
    ...
    if open_chat_id == nil || open_chat_id != message.chat_id do
      {:noreply,
       socket |> assign(:unread_messages, Chats.get_count_unresponded_chat_users(current_user.id))}
    end
