defmodule TalarWeb.ChatLiveTest do
  ...
  defp create_chat(_) do
    user_1 = user_fixture(%{email: "user1@email.com", username: "user1"})
    user_2 = user_fixture(%{email: "user2@email.com", username: "user2"})

    _chat = chat_fixture(%{user_id_1: user_1.id, user_id_2: user_2.id})
    :ok
  end
  ...
  describe "Index" do
    ...
    test "lists all chats", %{conn: conn} do
      session = %{"current_user" => %{"email" => "user1@email.com", "username" => "user1"}}
